...
function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--line)]/80 bg-[var(--surface)]/85 backdrop-blur-md transition-colors duration-300">
      <nav className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-3.5 lg:px-10">
        <a
          href="#home"
          className="text-[12px] font-medium uppercase tracking-[0.2em] text-[var(--text)] transition-colors duration-300 ease-out hover:text-[var(--accent)]"
        >
          BERITA
        </a>
        <ul className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
            <li key={link.label}>
              <a
                href={link.href}
                className="text-sm font-normal text-[var(--text-muted)] transition-colors duration-300 ease-out hover:text-[var(--text)]"
              >
                {link.label}
              </a>
            </li>
